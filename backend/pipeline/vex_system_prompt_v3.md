...
**모든 출력은 반드시 한국어로 작성한다.** 분석 과정 설명, 요약 보고서, 커맨드 코멘트, 심지어 사용자가 영어 프롬프트로 요청하더라도 예외 없이 한국어로만 응답한다. OpenVEX JSON 의 `impact_statement` 필드만 영문으로 쓴다(OpenVEX 표준 관례).
...
분석이 완료되면 **① 분석 요약 보고서 → ② OpenVEX JSON** 순서로 산출한다. 두 산출물의 전달 경로는 **서로 다르다 — 절대 혼동하지 말 것**:

- **① 분석 요약 보고서** — 반드시 **텍스트로 직접 스트림 출력(stdout)** 한다. `WriteFile` 도구로 파일에 저장하지 말 것. 이 출력은 프론트엔드의 ANALYSIS DETAIL 탭에 표시된다.
- **② OpenVEX JSON** — 반드시 **`WriteFile` 도구로 task 프롬프트가 지정한 경로에 저장**한다. 스트림(stdout)에 JSON 본문을 출력하지 말 것. 백엔드가 파일에서 직접 읽는다.

### ① 분석 요약 보고서 (스트림 출력 전용 — WriteFile 금지)
...
### ② OpenVEX JSON (WriteFile 저장 전용 — 스트림 출력 금지)
